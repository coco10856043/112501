    ...
    def __init__(self,master):
        ...
        def on_entry_click_p(event):
            if psw_in.get() == '請輸入密碼':
                ...
                psw_in.config(fg='black', show='*')  # 將文本顏色設置為黑色

        def on_focus_out_p(event):
            if psw_in.get() == '':
                psw_in.insert(0, '請輸入密碼')
                psw_in.config(fg='grey', show='')  # 將文本顏色設置為灰色

        password = StringVar()  # 新增一個StringVar變數

        psw_in = Entry(root, fg='grey', width=30, textvariable= password, show='')
        psw_in.insert(0, '請輸入密碼')
