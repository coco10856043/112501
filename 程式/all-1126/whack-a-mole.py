...
# 要蓋上的圖片
if result_list[11] == '1':
    image_to_overlay = cv2.imread('game_png/Rat-1.png', cv2.IMREAD_UNCHANGED)  # 替換成您的圖片路徑
    image_to_overlay = cv2.resize(image_to_overlay, (100, 100))  # 調整圖片大小
elif result_list[11] == '2':
    image_to_overlay = cv2.imread('game_png/Ratt-1.png', cv2.IMREAD_UNCHANGED)  # 替換成您的圖片路徑
    image_to_overlay = cv2.resize(image_to_overlay, (100, 100))  # 調整圖片大小
else:
    image_to_overlay = cv2.imread('game_png/Rattt-1.png', cv2.IMREAD_UNCHANGED)  # 替換成您的圖片路徑
    image_to_overlay = cv2.resize(image_to_overlay, (100, 100))  # 調整圖片大小

if image_to_overlay.shape[2] == 4:  # 检查图像是否有 alpha 通道
    alpha_channel_r = image_to_overlay[:, :, 3] / 255.0
    overlay = image_to_overlay[:, :, :3]
else:
    alpha_channel_r = np.ones((image_to_overlay.shape[0], image_to_overlay.shape[1]), dtype=image_to_overlay.dtype)
    overlay = image_to_overlay
# image_hammer = cv2.imread('game_png/hammer-1.png', cv2.IMREAD_UNCHANGED)  # 替換成您的圖片路徑
# image_hammer = cv2.resize(image_hammer, (100, 100))  # 調整圖片大小
# if image_hammer.shape[2] == 4:  # 检查图像是否有 alpha 通道
#     alpha_channel_h = image_hammer[:, :, 3] / 255.0
#     hammer = image_hammer[:, :, :3]
# else:
#     alpha_channel_h = np.ones((image_hammer.shape[0], image_hammer.shape[1]), dtype=image_hammer.dtype)
#     hammer = image_hammer
if result_list[11] == '1':
    image_hammer = cv2.imread('game_png/hammer-1.png', cv2.IMREAD_UNCHANGED)
elif result_list[11] == '2':
    image_hammer = cv2.imread('game_png/paper-1.png', cv2.IMREAD_UNCHANGED)
else:
    image_hammer = cv2.imread('game_png/stick-1.png', cv2.IMREAD_UNCHANGED)

if image_hammer.shape[2] == 3:  # 如果是三通道，加入透明通道
    alpha_channel = np.ones((image_hammer.shape[0], image_hammer.shape[1]), dtype=image_hammer.dtype) * 255
    image_hammer = cv2.merge((image_hammer, alpha_channel))
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        if image_to_overlay is not None:
            ...
            alpha_channel_r = image_to_overlay[:, :, 3] / 255.0  # 提取透明通道并归一化
            overlay = image_to_overlay[:, :, :3]  # 提取RGB通道

            # 设置图像位置
            roi_r = image[img_y:img_y+100, img_x:img_x+100]

            # 将图像叠加到视频帧上
            for c in range(3):
                roi_r[:, :, c] = (1 - alpha_channel_r) * roi_r[:, :, c] + alpha_channel_r * overlay[:, :, c]
        ...
        if results.multi_hand_landmarks:
            for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                image = cvzone.overlayPNG(image, image_hammer, (x, y))
                 # 繪製圖片
                # if image_hammer is not None:                    
                #     # 將圖片蓋在畫面上
                #     # image[y:y+100, x:x+100] = image_hammer
                #     # 设置图像在相机捕获的视频帧的位置（考虑图像的透明度）
                #     alpha_channel_h = image_hammer[:, :, 3] / 255.0  # 提取透明通道并归一化
                #     hammer = image_hammer[:, :, :3]  # 提取RGB通道

                #     # 设置图像位置
                #     roi_h = image[y:y+100, x:x+100]

                #     # 將圖片叠加到视频帧上
                #     for c in range(3):
                #         roi_h[:, :, c] = (1 - alpha_channel_h) * roi_h[:, :, c] + alpha_channel_h * hammer[:, :, c]
        ...
        if timer_count == 0:
            ...
            query1 = "INSERT INTO game (`user_id`, `game_type`, `game_time`, `scores`) VALUES (%s, '2', current_timestamp(), %s);"
...
background_img = pygame.image.load("game_png/score_bg.png")
