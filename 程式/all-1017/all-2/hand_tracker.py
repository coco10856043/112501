...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        ...
        if hand_x < 0.33:
            ...
            if time.time() - start_time > 5:
                ...
                # 執行另一個 Python 檔案
                subprocess.run(["python", "whack-a-mole.py", result_from_get_account])
            ...
            if time.time() - start_time > 5:
                ...
                # 執行另一個 Python 檔案
                subprocess.run(["python", "run.py", result_from_get_account])
            ...
            if time.time() - start_time > 5:
                ...
                # 執行另一個 Python 檔案
                subprocess.run(["python", "catch-ball.py", result_from_get_account])
    ...
    if cv2.waitKey(1) & 0xFF == 27:
        ...
        cv2.destroyAllWindows()                
        # 執行另一個 Python 檔案
        subprocess.run(["python", "AR_home.py", result_from_get_account])
