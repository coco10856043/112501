    ...
    if results.multi_hand_landmarks:
        ...
        if hand_x < 0.33:
            ...
            if time.time() - start_time > 5:
                ...
                # 在這裡執行資料庫查詢
                query = "SELECT * FROM user_information WHERE id = %s"
                values = (result_list[0])
                
                # 執行查詢
                initface_cursor1.execute(query, values)
                # 獲取查詢結果
                result = initface_cursor1.fetchall()
                ...
                # 執行另一個 Python 檔案
                subprocess.run(["python", "whack-a-mole.py", str(result)])
            ...
            if time.time() - start_time > 5:
                ...
                # 執行另一個 Python 檔案
                subprocess.run(["python", "run.py", str(result)])
            ...
            if time.time() - start_time > 5:
                ...
                # 執行另一個 Python 檔案
                subprocess.run(["python", "Pose.py", str(result)])
